  ...
  return (
    <div className="clamp-[px,1.5rem,5rem] text-black relative clamp-[py,2.5rem,7.5rem]">
      <div className="flex lg:flex-row flex-col clamp-[text,12px,16px] clamp-[leading,18px,25px]  clamp-[gap,20px,50px]">
        <div className="clamp-[min-w,10.1875rem,25rem] sticky flex flex-col justify-between  clamp-[top,4.5rem,8rem] z-50 bg-white  shrink-0">
          <div className="h-full">
            ...
            <Disclosure>
              {({ open }) => (
                <div>
                  <DisclosureButton
                    className={`flex justify-between clamp-[pt,13px,20px]  items-center w-full  text-black focus-visible:ring-opacity-75 `}
                  >
                    <span>Tags</span>
                    {open ? (
                      <DownAngleArrow fill="black" />
                    ) : (
                      <DownAngleArrow fill="black" className="rotate-180" />
                    )}
                  </DisclosureButton>
                  <DisclosurePanel>
                    {datas.map((item, i) => (
          ...
          <Link
            href={"/policy"}
            className="flex w-fit hover:underline decoration-1 underline-offset-4 items-center"
          >
            {" "}
